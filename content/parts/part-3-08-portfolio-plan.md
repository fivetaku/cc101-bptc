...
3. 작업 폴더 진입 (`cd`)
4. `progress.json` 갱신 — `current_clip = "Part03-Clip8-포트폴리오기획"`
5. 셋업 결과 한 줄 보고

> **이전 Clip 5~7 진행 여부와 무관하게 작동해요.** Clip 8은 김지수 자료만 필요하고, 이전 클립 결과물(리서치·카드뉴스·영상)과 분리된 실습입니다.

### 2단계. Clip 8 시작 (위 보고 받은 다음에 입력)

```text
방금 셋업한 실습11-포트폴리오기획/ 폴더의 김지수 리서치(00-포트폴리오-리서치.md)와 이미지 15장(images/)으로 포트폴리오 웹사이트를 만들려고 합니다. Clip 8 (포트폴리오 기획)을 시작할게요. 먼저 던지기 단계 — 이 자료로 사이트를 만들려면 어떤 순서로 진행하면 좋을까요? 기획서 작성부터 HTML 생성까지 흐름을 알려주세요. 바로 HTML부터 만들지 말고, 흐름 옵션을 먼저 설명해 주세요.
```

→ 이 프롬프트가 Clip 8의 **던지기(1단계)** 진입점입니다. 클로드코드가 흐름 옵션을 펼쳐주면 다음 진행 흐름 2~5단계로 이어집니다.

### 셋업 완료 체크 (1단계 입력 후 확인)
