...
이 클립은 **가상 인물 김지수**의 포트폴리오 자료를 사용합니다. 워크스페이스 안 `30-templates/portfolio-template/`에 미리 들어 있어요. 작업 폴더로 옮겨두면 바로 시작할 수 있어요.

### 1단계. 자료 셋업 (준비만 — 클로드코드에 입력)

```text
~/Desktop/bptc-cc/30-templates/portfolio-template/ 안의 자료(00-포트폴리오-리서치.md + images/)를 ~/Desktop/bptc-cc/50-my-work/Part03-체험하기/실습11-포트폴리오기획/ 폴더로 복사하고, 그 작업 폴더로 진입해 주세요. 셋업이 끝나면 작업 폴더 안 파일 목록을 한 줄로 보고만 해 주세요. 실습은 아직 시작하지 마세요 — 다음 프롬프트를 받고 시작합니다.
```

→ 클로드코드가 자동으로:
1. `mkdir -p 50-my-work/Part03-체험하기/실습11-포트폴리오기획/`
2. `cp -r 30-templates/portfolio-template/* 실습11-포트폴리오기획/`
3. 작업 폴더 진입 (`cd`)
4. 파일 목록 한 줄 보고 후 **대기**

이 명령은 **준비(폴더·자료 셋업)만**입니다. 클립 8 본격 시작은 다음 2단계 프롬프트로.
...
- `00-포트폴리오-리서치.md` (김지수 프로필 + 레퍼런스 15개 + 디자인 트렌드)
- `images/` (이미지 15장)

자료가 안 보이면 강사에게 "30-templates 폴더에 portfolio-template 있나요?" 확인 요청.

### 2단계. Clip 8 시작 (셋업 보고 받은 다음에 입력)

```text
이제 Clip 8 (포트폴리오 기획)을 시작합니다. 방금 셋업한 실습11-포트폴리오기획/ 폴더의 김지수 리서치(00-포트폴리오-리서치.md)와 이미지 15장(images/)으로 포트폴리오 웹사이트를 만들려고 합니다. 먼저 던지기 단계 — 이 자료로 사이트를 만들려면 어떤 순서로 진행하면 좋을까요? 기획서 작성부터 HTML 생성까지 흐름을 알려주세요. 바로 HTML부터 만들지 말고, 흐름 옵션을 먼저 설명해 주세요.
```

→ 이 프롬프트가 **Clip 8 던지기(1단계) 진입점**입니다. 클로드코드가 흐름 옵션을 펼쳐주면 아래 진행 흐름 2~5단계로 이어집니다.
