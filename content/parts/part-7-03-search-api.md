...
공공 API(Clip 2)와 검색 API(Clip 3)로 데이터를 받아오는 법을 익혔고, Clip 1에서 DB를 만들었어요. 마지막 클립에서 이 둘을 합칩니다. **API로 받아온 데이터를 진짜 데이터베이스에 적재**하고, 오후에 했던 간이 해커톤처럼 "API + DB" 한 덩어리로 굴려봅니다.
